...
class TestExtraction(unittest.TestCase):
    ...
    def test_should_raise_parsing_exception(self):
        ...
        with self.assertRaises(ParsingFailureException):
            OctopusParser(content_parsing_offers_failure, "test").get_offers()

        content_parsing_button_failure = "Le nostre tariffe luce"
        with self.assertRaises(ParsingFailureException):
            OctopusParser(content_parsing_button_failure, "test").get_next_page_button_url()

    def test_should_return_three_plan_offer_from_first_page(self):
        plan_offers = OctopusParser(page_one_content, "test").get_offers()
    ...
    def test_should_be_true_if_there_is_at_least_one_offer_under_100(self):
        ...
        self.assertTrue(OctopusParser(page_one_content, "test").should_download_all_pdfs(plan_offers))
    ...
    def test_should_be_false_if_offers_are_above_100(self):
        ...
        self.assertFalse(OctopusParser(page_one_content, "test").should_download_all_pdfs(plan_offers))

        plan_offers= [PlanOffer(commercial_cost=100)]
        self.assertFalse(OctopusParser(page_one_content, "test").should_download_all_pdfs(plan_offers))

    def test_should_return_button_url(self):
        self.assertEqual("/offerta/tariffe", OctopusParser(page_one_content, "test").get_next_page_button_url())
    ...
    # @patch('extraction.parser.OctopusParser._download', DownloadException("test","test error","test"))
    # def test_should_raise_download_exception(self):
    #     with self.assertRaises(DownloadException):
    #         with mock.patch('extraction.parser.OctopusParser._download') as mock_download:
    #             mock_download.side_effect = DownloadException("test", "test error", "test")
    #             # OctopusParser(page_one_content, "test")._download_files()
    #             with mock.patch('extraction.parser.OctopusParser.get_offers') as mock_offers:
    #                 mock_offers.return_value = [PlanOffer(), PlanOffer(), PlanOffer()]
    #                 OctopusParser(page_one_content, "test")._download_files()
    #
    #         # parser_mock = Mock(spec_set=OctopusParser, _download=Mock(side_effect=DownloadException("test","test error","test")))
    #         # seal(parser_mock)
    #         #
    #         # OctopusParser(page_one_content, "test")._download_files()
    ...
    def test_should_raise_download_exception(self):
        with self.assertRaises(DownloadException):
            ...
            OctopusParser._download(plan_offer)

    def test_should_return_three_plan_offer_from_second_page(self):
        plan_offers = OctopusParser(page_two_content, "test").get_offers()
